...
class cursive_json_func_data extends external_api {
        ...
        if (!isset($data['filename'])) {
            $sql = 'SELECT filename, id, userid
                     ...
                     WHERE userid = :userid
                            AND cmid = :cmid
                            AND modulename = :modulename';
            $filename = $DB->get_record_sql($sql, ['userid' => $params['id'], 'cmid' => $params['cmid'], 'modulename' => $params['modulename']]);
    ...
    public static function cursive_get_writing_differencs($fileid) {
        ...
        $filename = $DB->get_record('tiny_cursive_files',['id' => $vparams['fileid']],'filename');
